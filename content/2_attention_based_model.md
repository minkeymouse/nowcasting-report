...
|--------|-------------------|--|-------------------|--|
| | sMSE | sMAE | sMSE | sMAE |
| TFT | 3.48 | 1.58 | 1.11 | 0.85 |
| PatchTST | 2.14 | 1.28 | 1.62 | 0.94 |
| iTransformer | 2.22 | 1.33 | 0.89 | 0.75 |

(plot here)[combined_attention_forecast.png]

□ 단기예측 결과 해석
◦ TFT 기준으로 생산 지표에서는 회사채 3년(AA-) 금리, 뉴스심리지수, 실업률(전체, 계절조정), 국고채 10년 금리의 중요도가 높았음. 투자 지표에서는 취업자 수(건설업), 생산(건설업), 투자 전망(전산업), 취업자 수(광공업)의 어텐션 가중치가 높게 나타남
◦ iTransformer의 경우 투자 지표에서 예측의 강점을 보였는데, 데이터 특성상 상대적으로 정보가 많고 변수간 상관관계가 다양한 점이 작용함
◦ PatchTST는 생산 지표에서 비교적 안정적인 오차 수준을 보였으나 투자 지표에서는 성능 저하가 나타남
