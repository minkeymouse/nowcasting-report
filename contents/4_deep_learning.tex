...
심층 동적 요인 모형(Deep Dynamic Factor Model, DDFM)은 딥러닝 기법을 DFM에 접목한 모형으로, 변분 자기인코더(Variational Autoencoder, VAE)를 활용하여 비선형 요인 구조를 학습한다 \cite{andreini2020deep}. DDFM은 기존 DFM의 선형 가정을 완화하여 더 복잡한 요인 구조를 포착할 수 있다.
...
\subsubsection{변분 자기인코더 기반 요인 추출}
DDFM은 변분 추론(variational inference)을 통해 요인의 사후 분포를 근사한다. 변분 자기인코더(VAE)는 확률적 인코더 $q_\phi(z|x)$와 확률적 디코더 $p_\theta(x|z)$로 구성되며, 변분 하한(variational lower bound)을 최대화함으로써 학습된다.
...
여기서 첫 번째 항은 재구성 오차(reconstruction error)를, 두 번째 항은 사전 분포 $p(z)$와의 KL 발산을 나타낸다. KL 발산 항은 정규화 역할을 하여, 요인이 의미 있는 표현을 학습하도록 유도한다.
...
\paragraph{실증적 증거}
한국 GDP 나우캐스팅 연구 \cite{kim2024deep}에 따르면, 선형 DFM은 전체 기간(1985-2019) 동안 MAE 3.9\%를 달성했으나, 2020년 2-3분기(COVID-19 팬데믹) 동안 성능이 크게 저하되었다. 반면 DDFM은 전체 기간 동안 MAE 2.2\%를 달성하여 44\%의 개선을 보였으며, 특히 변동성이 큰 시기에 우수한 성능을 보였다.
...
\begin{itemize}
    \item \textbf{아키텍처}: TFT는 트랜스포머 기반이며, DDFM은 VAE 기반이다.
...
[실험 결과에 따라 DFM vs DDFM 성능 비교 작성]

\subsubsection{비선형 관계 포착 능력}
DDFM이 비선형 관계를 포착하는 능력을 검증하기 위해, 변동성이 큰 변수나 구조적 변화가 있는 시기에 대한 예측 성능을 분석한다.

[실험 결과에 따라 비선형 관계 포착 능력 분석 작성]

\subsubsection{나우캐스팅 성능}
DDFM의 나우캐스팅 성능을 마스킹된 데이터를 활용한 백테스팅을 통해 평가한다.

[실험 결과에 따라 DDFM의 나우캐스팅 성능 분석 작성]

\subsection{Ablation Study}
\label{sec:ablation}

\subsubsection{하이퍼파라미터 영향 분석}
DDFM의 각 하이퍼파라미터가 모형 성능에 미치는 영향을 분석하기 위해 Ablation study를 수행한다. 주요 하이퍼파라미터는 다음과 같다:

\begin{itemize}
    \item 인코더 레이어 수: 1, 2, 3, 4 레이어
    \item 요인 개수: 1, 2, 3, 4, 5개
    \item 학습률: 0.001, 0.01, 0.1
    \item 배치 크기: 16, 32, 64, 128
\end{itemize}

각 하이퍼파라미터를 체계적으로 변화시키며 모형 성능을 측정함으로써, 최적의 하이퍼파라미터 조합을 찾고 각 파라미터의 영향을 정량적으로 평가한다.

[실험 결과에 따라 Ablation study 결과 작성]

\subsubsection{DFM 하이퍼파라미터 분석}
DFM의 요인 개수와 AR 차수 변화에 따른 성능 분석도 수행한다.

[실험 결과에 따라 DFM Ablation study 결과 작성]
