...
선형 DFM은 다음과 같은 다섯 가지 핵심 가정에 기반한다 \cite{andreini2020deep}:
\begin{enumerate}
    \item \textbf{선형 인코딩 함수}: 인코딩 함수 $G(\cdot): \mathbf{y} \to \mathbf{f}$가 선형 연산자이다.
    \item \textbf{선형 디코딩 함수}: 디코딩 함수 $F(\cdot): \mathbf{f} \to \tilde{\mathbf{y}}$가 선형 연산자이다.
    \item \textbf{선형 요인 동학}: $z_t = A z_{t-1} + w_t$ - 요인이 선형 확률 벡터 차분 방정식을 따른다.
    \item \textbf{선형 특이 성분 동학}: 특이 성분이 대각 행렬의 자기회귀 계수를 가진 선형 확률 벡터 차분 방정식을 따른다.
    \item \textbf{가우시안 오차}: 전이 방정식과 관측 방정식의 오차 항이 독립 동일 정규분포(i.i.d. Gaussian)를 따른다.
\end{enumerate}
...
\paragraph{학습 과정}
DDFM의 학습은 MCMC(Markov Chain Monte Carlo) 기반 알고리즘을 사용하며, 두 단계로 구성된다 \cite{andreini2020deep}:

\begin{enumerate}
    \item \textbf{오프라인 학습(Off-line estimation)}: 전체 모형의 파라미터를 추정한다. 이 단계에서는 사전 학습(pre-training)을 통해 자기인코더를 초기화하고, 이후 MCMC 알고리즘을 통해 인코더, 디코더, 특이 성분 파라미터를 교대 최적화한다. Denoising 접근법을 사용하여 노이즈 주입을 통해 모형의 강건성을 향상시킨다.
    \begin{equation}
    \min_{\phi, \theta, \Phi, \Sigma_\varepsilon} \sum_{t=1}^T ||x_t - f_\theta(g_\phi(x_t^{(mc)}))||^2
    \end{equation}
    여기서 $x_t^{(mc)}$는 노이즈가 주입된 관측치이며, 특이 성분 파라미터 $\Phi$와 $\Sigma_\varepsilon$는 요인 추정과 교대하여 업데이트된다.
    
    \item \textbf{온라인 학습(On-line updates)}: 디코딩 부분을 상태 공간 프레임워크로 변환하여 관측치가 주어질 때 잠재 상태를 실시간으로 업데이트한다. Kalman 필터를 사용하여 ragged edges와 결측치를 처리한다.
\end{enumerate}
...
미니배치 학습을 통해 대규모 데이터셋에서도 효율적으로 학습할 수 있으며, 결측치는 selection matrices를 사용하여 손실 함수에서 마스킹하여 자연스럽게 처리한다. Denoising 접근법을 통해 노이즈 주입을 사용하여 모형의 강건성을 향상시킨다. 이는 EM 알고리즘과 달리 그래디언트 기반 최적화의 장점이다.
...
\paragraph{실증적 증거}
한국 GDP nowcasting 연구 \cite{kim2024deep}에 따르면, Mamba 모형(딥러닝 기반 상태공간모형)이 선형 DFM보다 우수한 성능을 보였으며, 특히 변동성이 큰 시기와 구조적 변화 시기에 더 나은 성능을 보였다고 보고됨. 이는 비선형 딥러닝 모형이 구조적 변화 시기에 선형 DFM보다 우수한 성능을 보일 수 있음을 시사한다.
...
\begin{itemize}
    ...
    \item \textbf{시간 변동 적재}: 변수 간 상관관계가 급증하여 적재 $\Lambda$가 증가했으나, 선형 DFM은 고정된 $\Lambda$를 사용한다.
...
\paragraph{UAT (Universal Approximation Theorem)}
UAT(Universal Approximation Theorem)에 따르면, 충분히 넓은 하나의 은닉층을 가진 신경망은 임의의 연속 함수를 임의의 정확도로 근사할 수 있다. 이는 충분한 데이터와 계산 자원이 주어지면, DDFM이 임의로 복잡한 비선형 관계를 학습할 수 있음을 보장한다.
...
\begin{itemize}
    ...
    \item \textbf{상태 전이 모델}: 요인의 시간적 동학을 모델링하는 AR 모델로, 선형 전이 행렬 $A$를 사용한다. 특이 성분도 AR(d) 과정으로 모델링되며, 대각 행렬의 자기회귀 계수를 가진다.
...
\begin{enumerate}
    ...
    \item 손실 계산: 재구성 오차와 동학 오차의 가중합을 계산하여 손실을 구한다.
...
DDFM과 기존 DFM의 예측 성능을 비교한 결과, 목표 변수에 따라 성능 차이가 나타났다. KOGDP...D에서는 두 모형이 유사한 성능을 보였으며(DFM: sRMSE=0.713, 0.354; DDFM: sRMSE=0.706, 0.361), KOGFCF..D에서는 두 모형 모두 높은 오차를 보였으나 DDFM이 DFM보다 상대적으로 낮은 오차를 보였다(DFM: sRMSE=7.965, 8.870; DDFM: sRMSE=1.284, 2.189). KOCNPER.D에서는 DFM이 수치적 불안정성으로 인해 실패하였으나, DDFM은 성공적으로 예측을 수행하였다(sRMSE=0.456, 0.802). 전체적으로 DDFM이 DFM보다 우수한 성능을 보였으며(sRMSE: 0.9663 vs 4.4755), 특히 KOCNPER.D와 KOGFCF..D에서 DDFM의 우수성이 두드러졌다. 이는 비선형 요인 구조 학습이 일부 목표 변수에서 효과적임을 시사한다. 자세한 결과는 섹션 \ref{sec:results}의 표 \ref{tab:overall_metrics}를 참조하라.

\subsubsection{비선형 관계 포착 능력}
DDFM이 비선형 관계를 포착하는 능력을 검증하기 위해, 변동성이 큰 변수나 구조적 변화가 있는 시기에 대한 예측 성능을 분석할 수 있다. 본 연구에서는 전체 시계열에 대한 예측 성능만 평가하였으며, 구조적 변화 시기(예: COVID-19 팬데믹)에 대한 세부 분석은 향후 연구 과제로 남겨둔다. 다만, DDFM이 KOCNPER.D에서 DFM의 수치적 불안정성을 극복하고 성공적으로 예측을 수행한 것은 비선형 인코더가 복잡한 요인 구조를 학습할 수 있음을 시사한다. 또한 KOGFCF..D에서 DDFM이 DFM보다 상대적으로 낮은 오차를 보인 것도 비선형 관계 포착 능력의 효과로 볼 수 있다.

\subsubsection{Nowcasting 성능}
DDFM의 예측 성능을 혼합 빈도 데이터를 활용하여 평가함. 현재 표 \ref{tab:nowcasting_metrics}에 제시된 결과는 일반 예측 성능을 사용 가능한 예측 기간(1일, 7일)에 대해 집계한 결과이다. 자세한 내용은 섹션 \ref{sec:results}를 참조하라.
