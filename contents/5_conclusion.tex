...
\begin{itemize}
    \item \textbf{예측 실험:} 예측 실험에서 평가된 모형(ARIMA, VAR) 중에서는 VAR이 세 대상 변수 모두에서 최고 성능을 보임. KOIPALL.G에서는 VAR(sMAE=0.85), KOEQUIPTE에서는 VAR(sMAE=0.85), KOWRCCNSE에서는 VAR(sMAE=0.83)이 가장 우수함. DFM과 DDFM은 예측 실험에서 평가되지 않았음.
...
\begin{itemize}
    \item 예측 실험에서 VAR이 ARIMA 대비 세 대상 변수 모두에서 우수한 성능을 보임을 확인함.
