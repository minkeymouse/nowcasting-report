...
dfm-python은 고차원 시계열 데이터의 나우캐스팅과 예측을 위한 동적 요인 모형(Dynamic Factor Model, DFM)의 포괄적인 Python 구현체임. 본 연구에서는 dfm-python 패키지를 활용하여 DFM과 DDFM(Deep Dynamic Factor Model) 모형을 구현함.
...
\begin{itemize}
    ...
    \item \textbf{나우캐스팅 및 예측}: 모든 예측 기간에 대한 예측을 생성할 수 있음
...
\begin{itemize}
    ...
    \item \textbf{nowcast 모듈}: 나우캐스팅 기능 및 News decomposition
...
\begin{itemize}
    ...
    \item 동적 요인 모형으로는 dfm-python 패키지를 활용하여 DFM과 DDFM을 구현함
\end{itemize}
...
dfm-python의 나우캐스팅 기능을 활용하여 마스킹된 데이터를 통한 백테스팅을 수행할 수 있도록 설계함. 각 시점에서 목표 변수의 최근 관측치를 마스킹하고, 사용 가능한 고빈도 데이터만을 활용하여 예측을 수행하도록 설계함. 또한 DFM과 DDFM의 각 하이퍼파라미터가 모형 성능에 미치는 영향을 분석하기 위해 Ablation study를 수행할 수 있도록 설계함. 현재 실험 결과가 확보되지 않아 구체적인 분석은 제한적임.
