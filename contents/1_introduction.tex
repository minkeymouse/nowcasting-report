...
동적요인모형(DFM)은 많은 시계열에서 공통 요인을 추출하여 차원을 축소하고 혼합주기 데이터를 처리하는 기법임 \cite{stock2002forecasting}. DFM은 state-space 형태로 표현되며, Kalman filter를 사용하여 추론할 수 있어 혼합주기(mixed frequency) 데이터와 비동기적 데이터 발표를 자연스럽게 처리할 수 있음. 이는 nowcasting에 특히 적합한 특성으로, 실시간 데이터 흐름의 불규칙성(jagged edges)을 효과적으로 다룰 수 있음 \cite{banbura2012nowcasting}.
