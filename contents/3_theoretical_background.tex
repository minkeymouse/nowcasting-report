...
저빈도 시계열(예: 분기별 GDP)을 고빈도 요인(예: 월간 요인)으로부터 생성하기 위해 텐트 커널(tent kernel) 집계 방식을 사용함. 이는 FRBNY Staff Nowcast에서 사용되는 핵심 기법으로, 혼합 빈도 데이터를 효과적으로 처리하기 위한 방법임 \cite{bok2017macroeconomic}.
\begin{itemize}
    \item 텐트 커널은 시간 가중치를 부여하여, 분기 내 각 월의 기여도를 다르게 설정함. 이는 저빈도 관측 변수를 고빈도 잠재 상태로 매핑하는 결정론적 방법임
