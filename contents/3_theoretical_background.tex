...
\subsubsection{Diffusion 모델의 활용}
최근 연구에서는 확산 모델(diffusion models)을 활용하여 혼합 빈도 데이터의 복잡한 패턴을 학습하고 예측 성능을 향상시키는 방법이 제안되고 있음.
\begin{itemize}
    \item Diffusion 모델은 생성 모델의 한 종류로, 노이즈를 점진적으로 제거하여 데이터를 생성하는 과정을 역으로 학습함
    \item 혼합 빈도 데이터 처리에서 diffusion 모델은 고빈도 데이터의 노이즈를 제거하고, 저빈도 목표 변수와의 관계를 더 정확하게 모델링할 수 있는 잠재력을 보유함
    \item 이러한 접근법은 특히 비선형 관계와 복잡한 패턴을 포착하는 데 유용함
\end{itemize}
...
저빈도 시계열(예: 분기별 GDP)을 고빈도 요인(예: 월간 요인)으로부터 생성하기 위해 텐트 커널(tent kernel) 집계 방식을 사용함. 텐트 커널은 Mariano와 Murasawa (2003)에 의해 제안된 방법으로, 혼합 빈도 데이터를 효과적으로 처리하기 위한 핵심 기법임 \cite{mariano2003new}. 이 방법은 FRBNY Staff Nowcast에서도 핵심적으로 사용되며, Bańbura 등 (2012)과 Bok 등 (2017, 2019)의 연구에서 실증적으로 검증됨 \cite{bok2017macroeconomic}.
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{실증적 우수성}: Bańbura 등 (2012)과 Bok 등 (2017)의 연구에서 텐트 커널이 균등 가중치나 최근 가중치보다 우수한 성능을 보이는 것으로 보고됨 \cite{banbura2012nowcasting, bok2017macroeconomic}. 특히 FRBNY Staff Nowcast에서 텐트 커널을 핵심 기법으로 사용하여 실증적으로 검증됨 \cite{bok2019frbny}
    \end{itemize}
...
\subsubsection{VAE를 활용한 비선형 요인 추출}
DDFM은 VAE를 활용하여 비선형 요인 구조를 학습함 \cite{andreini2020deep}.
\begin{itemize}
