...
결과를 보면, DDFM 모형이 DFM 모형보다 전반적으로 우수한 성능을 보이며, 특히 KOIPALL.G에서 큰 성능 차이를 보임(DFM-4weeks sMSE: 16155.6 vs DDFM-4weeks sMSE: 81.7). 또한 예상대로 1주 전 예측이 4주 전 예측보다 대부분의 경우 더 정확함(DDFM-1week가 DDFM-4weeks보다 낮은 오차). 다만 KOIPALL.G의 경우 DFM 모형에서 1주 전 예측이 4주 전 예측보다 오히려 높은 오차를 보이는데, 이는 모형의 수치적 불안정성으로 인한 것으로 보임. DFM 모형의 경우 모든 22개월에 걸쳐 예측값이 단 2개의 고유값만을 보이는 반복적 예측 패턴이 관찰되었으며, 이는 극단적인 예측값이 클리핑 과정에서 경계값으로 수렴하여 발생한 문제임.
