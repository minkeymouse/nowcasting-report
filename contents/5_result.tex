...
\begin{itemize}
    ...
    \item ARIMA 모형은 의존성 문제(pmdarima 패키지 누락)로 인해 결과를 생성하지 못함
\end{itemize}
...
GDP 목표 변수에 대한 실험 결과를 보면, VAR 모형이 전체 평균에서 가장 우수한 성능을 보였으며(sRMSE=0.8209), DFM이 그 다음으로 좋은 성능을 보였음(sRMSE=0.8119). DDFM은 sRMSE=1.0511로 DFM보다 낮은 성능을 보였는데, 이는 빠른 테스트 파라미터(epochs=1)로 인한 과소 학습 가능성이 있음.

\input{tables/tab_overall_metrics}

표 \ref{tab:overall_metrics_by_target}는 목표 변수별 모형 성능을 보여줌. 현재 GDP에 대한 결과만 사용 가능하며, 민간 소비 및 총고정자본형성에 대한 실험은 진행 중임.

\input{tables/tab_overall_metrics_by_target}

표 \ref{tab:overall_metrics_by_horizon}는 예측 기간별 모형 성능을 보여줌. 1일 및 7일 예측 기간에 대해서는 유효한 결과가 있으나, 28일 예측 기간은 테스트 세트 크기 부족으로 인해 모든 모형에서 유효한 결과가 없음.
...
\begin{table}[h]
\centering
\caption{목표 변수별 nowcasting 성능 비교 (표준화된 RMSE)\footnote{Nowcasting 실험은 아직 구현되지 않았으며, 향후 마스킹된 데이터를 활용한 백테스팅을 통해 실제 nowcasting 성능을 평가할 예정임.}}
...
\begin{table}[h]
\centering
\caption{마스킹 기간별 nowcasting 성능 비교 (표준화된 RMSE)\footnote{Nowcasting 실험은 아직 구현되지 않았으며, 향후 마스킹된 데이터를 활용한 백테스팅을 통해 실제 nowcasting 성능을 평가할 예정임.}}
