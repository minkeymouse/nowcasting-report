...
\begin{itemize}
    \item dfm-python의 clock 프레임워크를 통해 월간 고빈도 지표들이 목표 변수들의 단기 변동을 잘 포착할 수 있었던 것으로 평가됨
    \item GDP 예측에서는 월간 생산지수, 수출입액 등이 중요한 역할을 하는 것으로 나타남
    \item 민간 소비 예측에서는 소비자 심리지수, 소매판매액 등 고빈도 지표들이 유용한 것으로 나타남
    \item 총고정자본형성 예측에서는 설비투자 관련 지표들이 예측에 유용한 것으로 나타남
    \item dfm-python의 블록 구조를 통해 각 목표 변수와 관련된 변수들을 Block\_Global, Block\_Consumption, Block\_Investment 블록으로 그룹화하여 요인을 추출할 수 있었음
    \item DDFM의 비선형 인코더를 통해 각 목표 변수와 관련된 고빈도 지표들의 비선형 관계를 효과적으로 학습할 수 있었음
    \item 특히 변동성이 큰 총고정자본형성 예측에서 비선형 관계를 학습할 수 있는 DDFM의 장점이 두드러지게 나타남
    \item dfm-python의 텐트 커널 집계 방식을 통해 분기별 목표 변수를 월간 요인으로부터 효과적으로 예측할 수 있었음
\end{itemize}
...
\begin{itemize}
    ...
    \item GDP와 민간 소비 예측에서는 DDFM이 가장 우수한 성능을 보였으며, 이는 각각 Block\_Global과 Block\_Consumption을 통해 관련 변수들의 공통 변동을 효과적으로 포착할 수 있기 때문인 것으로 평가됨
    \item 총고정자본형성 예측에서는 높은 변동성으로 인해 예측이 어려운 것으로 나타났으나, DDFM은 비선형 요인 구조 학습 능력으로 인해 상대적으로 안정적인 성능을 유지한 것으로 평가됨
...
\begin{itemize}
    ...
    \item GDP 예측에서는 장기 의존성을 학습할 수 있는 동적 요인 모형의 우수성이 두드러지게 나타남
    \item 민간 소비 예측에서는 소비자 심리와 밀접한 관련이 있어, DDFM이 변분 자기인코더를 통해 소비자 심리와 소비 간의 복잡한 비선형 관계를 학습하여 최우수 성능을 기록함
    \item 총고정자본형성 예측에서는 가장 큰 변동성을 보이므로 예측이 가장 어려운 것으로 나타났으나, 특히 비선형 관계를 학습할 수 있는 모형들이 상대적으로 우수한 성능을 보인 것으로 평가됨
    \item DDFM이 변분 자기인코더를 통해 투자의 비선형 동학을 효과적으로 학습하여 세 목표 변수 모두에서 최우수 성능을 기록함
    \item DFM도 세 목표 변수 모두에서 두 번째로 우수한 성능을 보였으며, 장기 의존성을 학습할 수 있는 동적 요인 모형의 우수성이 두드러지게 나타남
