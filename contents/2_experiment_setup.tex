...
\begin{itemize}
    \item 주요 모형(DFM, DDFM)은 다변량 고차원 데이터를 활용하는 반면, 고빈도 지표(전력거래량, BSI)의 단일변수 예측에서의 활용 가능성을 탐색하기 위해 실험을 수행함. 이 실험은 전산업생산지수(KOIPALL.G) 단일변수에 대해 MIDAS-AR(1), AR(1) 벤치마크, 선형 ARX, 그리고 XGBoost 기반 비선형 모형을 비교함. 월별 전산업생산지수를 종속변수로, 주별 전력거래량과 BSI를 설명변수로 사용하여 MIDAS-AR, ARX, XGBoost 모형의 성능을 비교함.
\end{itemize}
