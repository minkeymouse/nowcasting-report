...
\begin{itemize}
    ...
    \item Bańbura 등 (2012)은 DFM을 활용한 nowcasting 프레임워크를 제시하며, 실시간 데이터 흐름을 고려한 혼합 빈도 모형의 이론적 기초를 정립함 \cite{banbura2012nowcasting}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{텐트 커널 집계}: Mariano와 Murasawa (2003)는 텐트 커널(tent kernel) 집계 방식을 제안하여 저빈도 시계열을 고빈도 요인으로부터 생성하는 방법을 제시함 \cite{mariano2003new}. 텐트 커널은 분기 내 각 월의 기여도를 시간 가중치로 부여하여, 분기의 중간 시점이 더 큰 가중치를 갖도록 함. 이는 분기 중간 시점이 전체 분기 값을 더 잘 대표한다는 경제적 직관에 기반함. 이 방법은 FRBNY Staff Nowcast에서 핵심적으로 사용되는 기법으로, Bańbura 등 (2012)과 Bok 등 (2017, 2019)의 연구에서 실증적으로 검증됨 \cite{bok2017macroeconomic, bok2019frbny}
    \item \textbf{Diffusion 모델의 활용}: 최근 연구에서는 확산 모델(diffusion models)을 활용하여 혼합 빈도 데이터의 복잡한 패턴을 학습하고 예측 성능을 향상시키는 방법이 제안되고 있음. 이러한 접근법은 고빈도 데이터의 노이즈를 제거하고 저빈도 목표 변수와의 관계를 더 정확하게 모델링할 수 있는 잠재력을 보유함
    \item 분기별 GDP와 같은 저빈도 변수를 월간 또는 주간 고빈도 지표로부터 예측할 수 있으며, 이는 nowcasting에 매우 유용함
    \item Bańbura 등 (2012)은 nowcasting과 실시간 데이터 흐름에 대한 포괄적인 프레임워크를 제시하였으며, 혼합 빈도 동적 요인 모형을 활용한 nowcasting 방법론을 정립함 \cite{banbura2012nowcasting}
\end{itemize}
...
\begin{itemize}
    \item Bańbura 등 (2012)은 nowcasting의 이론적 기초를 정립하고, 실시간 데이터 흐름을 활용한 nowcasting 프레임워크를 제시함. 이들은 혼합 빈도 동적 요인 모형을 활용하여 분기별 GDP를 월간 고빈도 지표로부터 예측하는 방법론을 제시함 \cite{banbura2012nowcasting}
...
\begin{itemize}
    ...
    \item Huber 등 (2020)도 유사한 연구를 수행하여 혼합 빈도 VAR을 활용한 nowcasting에서 팬데믹 기간 동안의 성능 저하를 관찰함 \cite{huber2020nowcasting}
...
\begin{itemize}
    \item Kim (2024)은 딥러닝 방법을 활용한 거시지표 nowcasting 모형을 제안하였으며, DDFM이 DFM보다 우수한 성능을 보인 것으로 보고됨 \cite{kim2024deep}
