...
본 연구에서는 최대 반복 횟수를 100회로 설정하고, 수렴 기준을 $10^{-4}$로 설정하도록 설계함. EM 알고리즘은 일반적으로 50-200회 반복 내에 수렴하며, 초기화가 중요하다. 나쁜 초기값은 수렴 실패나 지역 최적해로의 수렴을 야기할 수 있다.
...
DFM의 nowcasting 성능은 마스킹된 데이터를 활용한 백테스팅을 통해 평가된다. 구체적으로, 과거 시점에서 최근 관측치를 마스킹하여 실제 nowcasting 상황을 시뮬레이션한다.
