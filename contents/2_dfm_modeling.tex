...
요인 적재 행렬 $C$의 각 원소 $C_{ij}$는 시계열 $i$가 요인 $j$에 얼마나 민감한지를 나타낸다. 전이 행렬 $A$는 요인이 시간에 따라 어떻게 진화하는지를 결정하며, $A$의 고유값(eigenvalue)의 절댓값이 1보다 작으면 요인은 정상 과정(stationary process)을 따른다.
...
이러한 목표 변수들은 분기별로 발표되며, 해당 분기 종료 후 약 25일 정도의 시차를 가지고 있다. 따라서 nowcasting을 통해 공식 발표 전에 현재 분기의 값을 추정하는 것이 가능하다.
...
본 연구에서는 최대 반복 횟수를 100회로 설정하고, 수렴 기준을 $10^{-4}$로 설정함. EM 알고리즘은 일반적으로 50-200회 반복 내에 수렴하며, 초기화가 중요하다. 나쁜 초기값은 수렴 실패나 지역 최적해로의 수렴을 야기할 수 있다.

\subsection{DFM 예측 결과}
\label{sec:dfm_results}

\subsubsection{예측 성능 평가}
DFM 모형의 예측 성능은 3개의 목표 변수와 3개의 예측 기간(1일, 7일, 28일)에 대해 평가되었다. 평가 지표는 표준화된 MSE, MAE, RMSE를 사용하며, 각 지표는 훈련 데이터의 표준편차로 정규화된다.

DFM 모형은 총 9개 조합 중 4개 조합에 대해 결과를 생성하였다. KOGDP...D에서는 상대적으로 양호한 성능을 보였으며(sRMSE=0.713, 0.354), KOGFCF..D에서는 높은 오차를 보였다(sRMSE=7.965, 8.870). KOCNPER.D는 EM 알고리즘의 수치적 불안정성으로 인해 모든 예측 기간에서 실패하였으며, 모든 목표 변수의 28일 예측은 테스트 세트 크기 부족으로 평가 불가능하였다. 자세한 결과는 섹션 \ref{sec:results}의 표 \ref{tab:overall_metrics} 및 표 \ref{tab:overall_metrics_by_target}를 참조하라.
...
본 연구에서는 요인의 경제적 해석에 대한 상세한 분석은 수행하지 않았으며, 이는 향후 연구 과제로 남겨둔다. 요인 해석을 위해서는 요인 적재 행렬의 분석, 요인 시계열의 시각화, 그리고 요인과 경제 변수 간의 상관관계 분석 등이 필요하다.

\subsubsection{Nowcasting 성능}
DFM의 nowcasting 성능은 마스킹된 데이터를 활용한 백테스팅을 통해 평가될 수 있다. 구체적으로, 과거 시점에서 최근 관측치를 마스킹하여 실제 nowcasting 상황을 시뮬레이션한다.

본 연구에서는 혼합 빈도 데이터를 활용한 예측 성능을 평가함. 현재 표 \ref{tab:nowcasting_metrics}에 제시된 결과는 일반 예측 성능을 사용 가능한 예측 기간(1일, 7일)에 대해 집계한 결과이다. 자세한 내용은 섹션 \ref{sec:results}를 참조하라.
