...
\begin{itemize}
  \item \textbf{표준화된 지표:} sMAE, sMSE. 두 지표 모두 훈련 데이터의 표준편차로 정규화하여 계산함. 변수 간 스케일 차이를 제거하여 직접 비교 가능하게 하며, 특히 거시경제 변수처럼 단위와 크기가 다른 변수들을 비교할 때 유용함 \cite{stock2002forecasting}.
  \item 모든 모형은 주간 단위로 학습하고 예측을 생성함. 주간 예측값을 월간으로 평균 집계한 후, 실제 월간 값과 비교하여 메트릭을 계산함. 지표는 1개월부터 22개월까지의 예측 시점에 대해 계산한 후 평균하여 최종 성능 지표로 사용함.
\end{itemize}
...
본 파트의 비교 실험에서는 변수별로 우수한 모형이 달랐으며(생산: Chronos, 투자: TFT, 소비: DDFM), 시점(단기/장기)에 따라서도 상대적 우위가 달라질 수 있음을 확인함. 또한 모형 간 성능 차이는 모델 아키텍처뿐 아니라 공변량 가용성, 결측치 처리, 예측 운영 방식(재귀/일괄) 등 실무적 설정에 의해 크게 좌우될 수 있음을 확인하였다. 따라서 후속 연구에서는 (i) 예측 시점별 정보집합(vintage) 기반의 엄밀한 설계, (ii) 안정화 강화, (iii) 동일한 튜닝·검증 예산 하의 공정 비교를 통해 결과의 재현성과 실시간 활용 가능성을 높일 필요가 있음.
