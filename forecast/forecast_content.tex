...
\begin{itemize}
  ...
  \item EM 알고리즘의 M-step에서 요인 공분산 행렬($\sum E[Z_t Z_t']$)의 조건수(condition number)가 반복에 따라 증가하면서 행렬 역행렬 계산 시 수치적 불안정성이 발생함. 조건수는 대칭 행렬의 경우 최대 고유값과 최소 고유값의 비율($|\lambda_{\max}| / |\lambda_{\min}|$)로 정의되며, 최소 고유값이 0에 가까워지거나 최대/최소 고유값의 비율이 커지면 조건수가 증가함. 특히 고정 정규화 계수(예: $10^{-6}$)는 초기 반복에서는 충분하지만, 조건수가 $10^8$ 이상으로 증가하면 불충분해져서 ill-conditioned 행렬 역행렬로 인한 수치적 오차가 누적됨.
  ...
  \item 선형 상태공간 모형의 구조적 한계로 인해 비선형 동학이나 구조변화를 포함한 데이터에서 근본적인 수렴 문제가 발생. 특히 거시경제 시계열은 위기, 팬데믹, 정책 변화 등으로 인한 비선형 동학과 구조변화를 포함하는 경우가 많으며 \cite{huber2020nowcasting}, 선형 가정 하에서 추정된 전이 행렬은 이러한 비선형 패턴을 포착하지 못하여 수렴 불안정성으로 이어질 수 있음 \cite{andreini2020deep, stock2016dynamic}.
\end{itemize}
