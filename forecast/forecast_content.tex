...
\begin{itemize}
  ...
  DFM의 수렴 이슈는 다음과 같은 다층적 원인에서 비롯됨. 첫째, 전이 행렬(transition matrix)의 스펙트럴 반경이 1보다 크면 상태 변수가 시간에 따라 기하급수적으로 발산하여 예측이 불안정해짐. 이는 EM 알고리즘의 M-step에서 전이 행렬을 추정할 때 안정성 제약이 충분히 강화되지 않아 발생할 수 있음. 둘째, EM 알고리즘의 M-step에서 요인 공분산 행렬($\sum E[Z_t Z_t']$)의 조건수(condition number)가 반복에 따라 증가하면서 행렬 역행렬 계산 시 수치적 불안정성이 발생함. 특히 고정 정규화 계수(예: $10^{-6}$)는 초기 반복에서는 충분하지만, 조건수가 $10^8$ 이상으로 증가하면 불충분해져서 ill-conditioned 행렬 역행렬로 인한 수치적 오차가 누적됨. 본 실험에서는 조건수에 비례하여 정규화 계수를 조정하는 적응적 정규화(adaptive regularization) 기법을 적용했으나, 일부 변수에서는 여전히 수렴 문제가 관찰되었음. 셋째, 데이터 품질 문제(높은 공선성, 극단적인 결측 패턴, 작은 유효 샘플 크기)가 수치적 불안정성을 악화시킴. 특히 결측치가 많은 경우 유효 관측값이 부족하여 요인 공분산 행렬 추정이 불안정해질 수 있음. 넷째, 선형 상태공간 모형의 구조적 한계로 인해 비선형 동학이나 구조변화를 포함한 데이터에서 근본적인 수렴 문제가 발생할 수 있음. 이러한 다층적 문제들로 인해 본 실험에서는 DFM에만 재귀적 예측(6개월 구간 단위)을 적용하여 발산을 완화했으나, 이는 예측 경로의 불연속성을 야기하고 모형 간 비교의 정합성을 저해함.
