...
변수별로 최우수 모형이 다르게 나타났으며, 특히 예측 시점(단기 vs 장기)에 따라 강점이 달라지는 양상이 관찰됨. 이는 (i) 타겟 변수의 변동성·구조변화 정도, (ii) 공변량의 정보량, (iii) 모형의 예측 방식(재귀 vs 원타임) 차이가 동시에 작용한 결과로 해석할 수 있음. 한편 DFM은 전이 행렬의 불안정성으로 인해 테스트 구간에서 오차가 크게 나타났음(논의 섹션 참조).
...
\begin{itemize}
  \item \textbf{생산(KOIPALL.G):} 생산지수는 비교적 완만한 추세·계절성이 존재하고, 다양한 공변량(서베이/금융/대외지표)이 광범위하게 연동되는 경향이 있어, 장기 의존성을 학습하거나(Transformer/RNN) 사전 학습 분포의 일반 패턴을 활용하는 모형(Chronos)이 유리할 수 있음. 반면 DDFM은 단기 구간에서 강점이 있었으나, 원타임 장기 예측에서는 시점이 길어질수록 오차가 커질 수 있음(잠재요인 동학의 단순화, 재귀 업데이트 부재 등).
  \item \textbf{투자(KOEQUIPTE):} 투자지수는 경기·금융여건 변화에 민감하고 변동성이 커서, 공변량의 단기 신호(금리/스프레드/서베이)를 잘 선택·가중하는 구조(TFT의 variable selection/attention)가 유리할 수 있음. 다만 이 결과는 ``예측 시점에 공변량이 실제로 관측 가능한가''에 민감하므로, 실시간 시나리오에 맞춘 입력 구성 점검이 필요함.
...
\begin{itemize}
  \item \textbf{DFM:} Kalman filter 기반 state-space 모형은 혼합주기/결측/비동기 발표를 구조적으로 처리할 수 있어 nowcasting에 적합하다 \cite{banbura2012nowcasting, bok2019frbny}. 그러나 본 실험에서는 전이 행렬 안정화가 충분하지 않아(스펙트럴 반경 $>1$) 예측 발산 또는 과도한 수렴이 관찰되었고, 이를 완화하기 위해 DFM에만 재귀 예측(청크 단위)을 적용하였다. 이로 인해 예측 경로의 불연속/들쭉날쭉함이 나타날 수 있으며, 비교의 공정성에도 영향을 준다.
...
\begin{itemize}
  ...
  \item \textbf{재귀 vs 원타임 예측}: DFM은 청크 재귀 예측, 다른 모형은 원타임 장기 예측 또는 다른 청크 길이로 운영되었기 때문에, ``모형 구조''뿐 아니라 ``예측 운영 방식''이 성능 차이의 일부를 설명할 수 있다. 후속 연구에서는 동일 모형에 대해 두 방식을 모두 비교하는 것이 바람직하다.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{예측 방식 차이}: 재귀/원타임 예측의 차이는 장기 예측에서 성능과 안정성에 직접 영향을 준다. 동일 모형에 대해 재귀 청크 길이와 업데이트 전략을 체계적으로 비교해야 한다.
\end{itemize}

\section{결론}
\label{sec:conclusion}

본 파트의 비교 실험에서는 변수별로 우수한 모형이 달랐으며(생산: Chronos, 투자: TFT, 소비: DDFM), 시점(단기/장기)에 따라서도 상대적 우위가 달라질 수 있음을 확인함. 또한 모형 간 성능 차이는 모델 아키텍처뿐 아니라 공변량 가용성, 결측치 처리, 예측 운영 방식(재귀/원타임) 등 실무적 설정에 의해 크게 좌우될 수 있음을 확인하였다. 따라서 후속 연구에서는 (i) 예측 시점별 정보집합(vintage) 기반의 엄밀한 설계, (ii) 안정화 강화, (iii) 동일한 튜닝·검증 예산 하의 공정 비교를 통해 결과의 재현성과 실시간 활용 가능성을 높일 필요가 있음.
