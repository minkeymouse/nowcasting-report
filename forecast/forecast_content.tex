...
\begin{itemize}
    ...
    \item \textbf{테스트 기간(Test):} 2024년 1월부터 2025년 10월까지 (22개월). 모든 모형의 예측 성능을 평가하는 기간으로, 실제 예측 상황을 시뮬레이션함. 각 모형은 훈련 기간 데이터로 학습한 후, 테스트 기간에 대해 예측을 생성함. 구체적으로, DFM은 발산 문제를 완화하기 위해 재귀적 예측(6개월 구간 단위)을 사용하며, DDFM은 발산 문제가 발생하지 않아 재귀적 예측 없이 한 번에 전체 22개월을 예측하는 일괄 예측 방식을 사용함.
\end{itemize}

\begin{itemize}
  \item 모든 모형은 주간 단위로 예측을 생성하며, 원본 타겟 변수가 월간 단위이므로 주간 예측을 월간으로 평균 집계하여 비교함.
...
\begin{itemize}
  \item \textbf{표준화된 지표:} sMAE, sMSE. 두 지표 모두 훈련 데이터의 표준편차로 정규화하여 계산함. 변수 간 스케일 차이를 제거하여 비교 가능하게 하며, 특히 거시경제 변수처럼 단위와 크기가 다른 변수들을 비교할 때 유용함 \cite{stock2002forecasting}.
...
\begin{itemize}
  \item \textbf{생산(KOIPALL.G):} 생산지수는 비교적 완만한 추세·계절성이 존재하고, 다양한 공변량(서베이/금융/대외지표)이 광범위하게 연동되는 경향이 있어, 장기 의존성을 학습하거나(Transformer/RNN) 사전 학습 분포의 일반 패턴을 활용하는 모형(Chronos)이 유리할 수 있음. 반면 DDFM은 단기 구간에서 강점이 있었으나, 일괄 장기 예측에서는 시점이 길어질수록 오차가 커질 수 있음(잠재요인 동학의 단순화, 재귀 업데이트 부재 등).
  \item \textbf{투자(KOEQUIPTE):} 투자지수는 경기·금융여건 변화에 민감하고 변동성이 커서, 공변량의 단기 신호(금리/스프레드/서베이)를 잘 선택·가중하는 구조(TFT의 변수 선택/attention)가 유리할 수 있음. 다만 이 결과는 ``예측 시점에 공변량이 실제로 관측 가능한가''에 민감하므로, 실시간 시나리오에 맞춘 입력 구성 점검이 필요함.
  \item \textbf{소비(KOWRCCNSE):} 소비는 다변량 공통요인 구조의 영향이 비교적 뚜렷할 수 있어, 고차원 공변량을 잠재요인으로 압축하고 상태공간 업데이트로 노이즈를 완화하는 요인 모형 계열(DDFM)이 안정적으로 작동할 가능성이 있음. 실제로 본 실험에서는 단기·장기 모두 DDFM의 우위가 관찰됨.
\end{itemize}

\subsection{논의}
\label{sec:discussion}

\subsubsection{4.2.1 모형별 강점과 실패 모드}

\begin{itemize}
  \item \textbf{DFM:} Kalman filter 기반 state-space 모형은 혼합주기/결측/비동기 발표를 구조적으로 처리할 수 있어 nowcasting에 적합하다 \cite{banbura2012nowcasting, bok2019frbny}. 그러나 본 실험에서는 전이 행렬 안정화가 충분하지 않아(스펙트럴 반경 $>1$) 예측 발산 또는 과도한 수렴이 관찰되었고, 이를 완화하기 위해 DFM에만 재귀 예측(구간 단위)을 적용하였다. 이로 인해 예측 경로의 불연속/들쭉날쭉함이 나타날 수 있으며, 비교의 공정성에도 영향을 준다.
  \item \textbf{DDFM:} 비선형 인코더로 잠재요인을 학습해 선형 가정의 한계를 일부 완화할 수 있으나 \cite{andreini2020deep}, 장기 예측에서는 잠재요인 동학(transition)의 단순화, 원타임 예측 설정, 그리고 구조변화 구간에서의 일반화 한계가 성능 저하로 나타날 수 있다. 반대로 단기 예측에서는 요인 추출의 이점이 두드러질 수 있다.
  \item \textbf{TFT/LSTM/Chronos:} 딥러닝 계열은 비선형성과 장기 의존성을 직접 학습할 수 있으나, (i) 데이터 양/전처리, (ii) 공변량의 가용성(실시간 예측 시점에 관측 가능한지), (iii) 직접 장기 예측 vs 재귀 예측의 선택에 따라 성능이 크게 달라진다. 특히 TFT는 공변량을 폭넓게 활용하는 만큼, 미래 공변량을 사실상 ``미리 알고'' 넣는 설정이 되면 성능이 과대평가될 수 있으므로 입력 설계 점검이 중요하다.
\end{itemize}

\subsubsection{4.2.2 평가 설계와 해석상의 주의점}

\begin{itemize}
  \item \textbf{주간→월간 평균 집계}: 모든 모형이 주간 단위로 예측을 생성한 뒤 월간 평균으로 평가되므로, 주간 내 변동을 잘 맞추는 모형의 이점이 평가에서 약화될 수 있다. 반대로 월간 평균 수준을 잘 맞추는 모형이 유리할 수 있다.
  \item \textbf{결측치 처리의 누수 가능성}: forward/backward fill 및 naive 채움은 구현 방식에 따라 미래 정보가 암묵적으로 섞일 수 있다. 특히 테스트 구간에서 backward-fill이 사용되었다면, 실시간 nowcasting 관점에서 부적절할 수 있으므로(미래값 사용), 파이프라인 점검이 필요하다.
  \item \textbf{재귀 vs 일괄 예측}: DFM은 구간 단위 재귀 예측, 다른 모형은 일괄 장기 예측 또는 다른 구간 길이로 운영되었기 때문에, ``모형 구조''뿐 아니라 ``예측 운영 방식''이 성능 차이의 일부를 설명할 수 있다. 후속 연구에서는 동일 모형에 대해 두 방식을 모두 비교하는 것이 바람직하다.
\end{itemize}

\subsection{의의 및 한계}
\label{sec:contribution_limitations}

\subsubsection{4.3.1 의의}
\begin{itemize}
  \item \textbf{동일 전처리 기반 비교}: state-space(DFM/DDFM)과 딥러닝(TFT/Chronos/LSTM)을 동일한 전처리 및 평가 방식 하에서 비교하여, 한국 거시변수 예측에서의 상대적 강점이 변수별로 달라질 수 있음을 보여주었다.
  \item \textbf{실용적 관찰 제공}: DFM의 수치적 불안정(전이행렬 안정성)과 같은 구현·운영 이슈가 실제 예측 성능을 크게 훼손할 수 있음을 명시적으로 확인하고, 안정화 강화 필요성을 제기하였다.
  \item \textbf{변수별 모델 선택의 근거}: 생산/투자/소비의 성격 차이에 따라 (i) 사전학습 기반 일반화, (ii) 공변량 선택/가중 구조, (iii) 잠재요인 기반 다변량 압축이 각기 다른 성능을 낼 수 있음을 실증 결과와 함께 정리하였다.
\end{itemize}

\subsubsection{4.3.2 한계 및 개선 방향}
\begin{itemize}
  \item \textbf{실시간(nowcast) 설정의 불완전성}: 예측 시점별로 사용 가능한 정보집합(vintage)을 엄밀히 구성하지 못했으며, 공변량이 예측 시점에 관측 가능한지에 대한 검증이 충분하지 않다. 후속 연구에서는 발표시차를 반영한 vintage 설계를 통해 데이터 누수를 방지해야 한다.
  \item \textbf{전처리·결측치 처리의 영향}: 결측치 채움 방식은 성능에 큰 영향을 줄 수 있으며, 실시간 예측 관점에서 허용되지 않는 정보 사용(backward-fill 등)이 있었는지 점검이 필요하다. 또한 robust scaling과 같은 변환이 각 모형에 미치는 영향도 분리해 평가할 필요가 있다.
  \item \textbf{하이퍼파라미터 튜닝의 비대칭}: 모형별 튜닝 자원이 동일하지 않으면 성능 비교가 왜곡될 수 있다. 동일한 튜닝 예산(탐색 공간/반복/검증 프로토콜)을 적용하는 것이 바람직하다.
  \item \textbf{예측 방식 차이}: 재귀/일괄 예측의 차이는 장기 예측에서 성능과 안정성에 직접 영향을 준다. 동일 모형에 대해 재귀 구간 길이와 업데이트 전략을 체계적으로 비교해야 한다.
\end{itemize}

\section{결론}
\label{sec:conclusion}

본 파트의 비교 실험에서는 변수별로 우수한 모형이 달랐으며(생산: Chronos, 투자: TFT, 소비: DDFM), 시점(단기/장기)에 따라서도 상대적 우위가 달라질 수 있음을 확인함. 특히 단기(1--6개월)에서는 DDFM의 강점이 관찰된 반면, 장기(13--22개월)에서는 생산·투자에서 딥러닝 계열(TFT/Chronos/LSTM)이 상대적으로 안정적인 경우가 나타났고, 소비에서는 DDFM 우위가 비교적 유지되는 경향이 확인되었음.

이 결과는 ``어떤 모형이 절대적으로 우수하다''라기보다, (i) 목표 변수의 변동성·구조변화 정도, (ii) 공변량의 정보량과 발표 시차, (iii) 전처리 및 결측치 처리, (iv) 예측 운영 방식(재귀/일괄)과 같은 실무적 설정이 성능을 크게 좌우한다는 점을 시사한다. 따라서 실시간 nowcasting 시스템 구축 관점에서는 모형 선택만큼이나 \emph{정보집합(vintage) 정의}와 \emph{데이터 누수 방지}, 그리고 \emph{예측 운영 방식의 표준화}가 중요함.

후속 연구 및 실무 적용을 위해서는 다음을 권고함:
\begin{itemize}
  \item \textbf{실시간 시나리오 정렬}: 예측 시점별로 사용 가능한 설명변수만 포함하는 vintage 설계를 적용하고, 결측치 처리에서 미래 정보가 사용되지 않도록 파이프라인을 재점검.
  \item \textbf{공정 비교 재설계}: 각 모형에 대해 재귀/일괄 예측을 모두 수행하고, 동일한 튜닝 예산(탐색 공간/검증 프로토콜) 하에서 성능을 비교.
  \item \textbf{DFM 안정화 강화}: 전이 행렬 안정화(규제·고유값 제약·초기화 전략)를 체계적으로 적용하여 발산/과수렴 문제를 완화하고, 안정화 전후 성능을 분리 보고.
  \item \textbf{강건성 점검}: 표본 분할(롤링/확장 윈도우), 위기 구간 별도 평가, 그리고 공변량 세트 변화에 대한 민감도 분석을 통해 결과의 일반화 가능성을 확인.
  \item \textbf{운영 전략}: 변수별로 강점이 다른 점을 활용해, (i) 변수별 최적 모형 선택 또는 (ii) 단순 앙상블(평균/가중평균) 기반의 베이스라인을 구축하고 예측 불확실성을 함께 관리.
\end{itemize}
