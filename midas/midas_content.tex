...
\begin{figure}[H]
    ...
    \caption{Vintage별 MIDAS–AR(1) 모형의 인샘플 적합 경로 (2002–2022). 종속변수는 전산업생산지수 월간 로그 성장률이며, 각 패널은 정보세트 $h0$–$h4$에서 검증 RMSE가 최소가 되도록 선택된 $K^{*}(h)$를 사용한 MIDAS–AR(1) 적합치를 나타낸다. 검은 실선은 실제값, 빨간 점선은 MIDAS–AR(1) 적합치를 의미한다.}
...
\begin{figure}[H]
    ...
    \caption{Vintage별 최종 추정 exp–Almon 가중치. 가로축은 주별 래그 인덱스 $k$ (1 = 가장 최근 주), 세로축은 가중치 $w_k^{(h)}$이며, 각 패널이 하나의 vintage 정보세트 ($h0$–$h4$)에 해당한다.}
...
\begin{figure}[H]
    ...
    \caption{테스트 구간(2023–2024)에서의 one-step-ahead 예측 경로. 각 패널은 하나의 vintage 정보세트($h0$–$h4$)에 대응하며, 검은 실선은 실제 월별 IIP 성장률, 파란 실선은 AR(1) 예측, 빨간 점선은 MIDAS–AR(1) 예측을 나타낸다. 각 패널 상단에는 해당 vintage에서의 RMSE(MIDAS, AR(1))가 표시되어 있다.}
...
\begin{figure}[H]
    ...
    \caption{전년동월비를 종속변수로 한 vintage별 MIDAS–AR(1)의 인샘플 적합 경로(2002–2022). 각 패널은 정보세트 $h0$–$h4$에서 선택된 $K^\ast(h)$와 추정 계수를 사용한 MIDAS–AR(1) 적합치를 나타낸다. 검은 실선은 실제 전년동월비, 빨간 점선은 MIDAS–AR(1) 적합치이다.}
...
\begin{figure}[H]
    ...
    \caption{전년동월비 모형에서의 vintage별 exp–Almon 가중치. 가로축은 주별 래그 인덱스 $k$ (1 = 가장 최근 주), 세로축은 가중치 $w_k^{(h)}$이며, 각 패널은 정보세트 $h0$–$h4$를 나타낸다.}
...
\begin{figure}[H]
    ...
    \caption{전년동월비를 종속변수로 한 테스트 기간(2023–2024)의 one-step-ahead 예측 경로. 각 패널은 하나의 vintage 정보세트(h0–h4)에 대응하며, 검은 실선은 실제 전년동월비, 파란 실선은 AR(1) 예측, 빨간 점선은 MIDAS–AR(1) 예측을 나타낸다. 패널 상단에는 vintage별 RMSE(MIDAS, AR(1))가 표시되어 있다.}
...
\end{tabular}
\begin{flushleft}
\caption{Vintage별 테스트 RMSE 및 AR(1) 대비 RMSE 감소율 (2023–2024)}
\footnotesize
...
\begin{figure}[H]
    ...
    \caption{전산업생산지수 성장률을 종속변수로 한 테스트 기간(2023–2024)의 one-step-ahead 예측 경로. 각 패널은 하나의 vintage 정보세트($h0$–$h4$)에 대응하며, 검은 실선은 실제 전산업생산지수 성장률, 초록 점선은 AR(1) 예측, 파란 점선은 AR(1)+XGB\_residual 예측, 노란색 점선은 ARX 예측, 분홍 점섬은 XGB-direct 예측을 나타낸다.}
...
\begin{figure}[H]
    ...
    \caption{전산업생산지수 변수 중요도 (feature importance) 히트맵}
...
\end{tabular}
\begin{flushleft}
\caption{ARX 모형 추정 결과: 월별 IIP 성장률에 대한 BSI 및 전력거래량의 영향}
\footnotesize
\textit{주}: 종속변수는 전산업생산지수 월별 성장률($y_t$)이며,
$y_{t-1}$은 1기 시차, $\text{pw}$는 월별(또는 주별) 전력거래량 관련 변수,
$\text{BSI}$는 기업경기실사지수, ``YoY''는 전년동월 대비 변화를 의미한다.
두 번째 열은 계수 추정치, 세 번째 열은 표준오차, 네 번째 열은 t값을 나타낸다.
\textit{***} $p<0.01$, \textit{**} $p<0.05$, \textit{*} $p<0.10$.
\end{flushleft}
\end{table}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $h0$에서만 AR(1) 대비 약 $4.4\%$의 RMSE 감소가 나타나, 전월까지의 정보만 사용할 수 있는 시점에서는 비선형 모형이 일정 부분 개선을 제공함.
    \item $h1$--$h4$에서는 감소율이 모두 음수(약 $-2.3\%\sim -8.6\%$)로, 고빈도 정보를 비선형으로 활용해도 AR(1)보다 예측오차가 커짐.
  \end{itemize}

  \item \textbf{종합}
  \begin{itemize}
    \item 전년동월비를 종속변수로 두었을 때, 대부분의 vintage에서 AR(1) 벤치마크가 가장 안정적인 선택임.
    \item 예외적으로 $h0$에서의 XGB-direct만 제한적인 개선을 보이며, 나머지 경우에는 고빈도 변수의 선형·비선형 확장이 예측력을 유의하게 개선하지 못함.
  \end{itemize}
\end{itemize}
...
\begin{figure}
    ...
    \caption{전년동월비를 종속변수로 한 테스트 기간(2023-2024)의 one-step-ahead 예측 경로. 각 패널은 하나의 vintage 정보세트($h0$-$h4$)에 대응하며, 검은 실선은 실제 전년동월비, 초록 점선은 AR(1) 예측, 파란 점선은 AR(1)+XGB\_residual 예측, 노란색 점선은 ARX 예측, 분홍 점선은 XGB-direct 예측을 나타낸다.}
    \label{fig:xgboost_test_mom_plots}
\end{figure}

\begin{itemize}
    \item AR(1)+XGB\_residual 모형은 몇몇 시점에서 진폭을 과도하게 확대해 실제값 주변의 노이즈까지 따라가는 모습이 관찰되며, 이는 앞선 RMSE 결과에서 확인된 성능 악화와 일관된 패턴이다.
